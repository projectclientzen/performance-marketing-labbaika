...
-- numbers; cs gets masked gross_profit/gross_booking_value from
-- get_cs_performance; a caller cannot pass another brand's id.
--
...
begin
  select brand_id into v_brand from rls_019_ids;
  select gross_profit into v_profit from get_dashboard_overview(v_brand, '2026-08-01', '2026-08-31', 'cash', null, null);
  if v_profit <> 3948000 then
    raise exception 'TEST FAILED: owner overview gross_profit=%, expected 3948000', v_profit;
  end if;
  raise notice 'TEST 3 PASSED: owner still sees correct gross_profit via get_dashboard_overview (%)', v_profit;
end $$;
...
begin
  ...
  for r in select * from get_cs_performance(v_brand, '2026-08-01', '2026-08-31') loop
    if r.cs_id = (select cs_id from rls_019_ids) then
      v_found := true;
      if r.gross_profit is not null or r.gross_booking_value is not null then
        raise exception 'TEST FAILED: cs sees gross_profit=% gross_booking_value=% (must be NULL)',
          r.gross_profit, r.gross_booking_value;
      end if;
      if r.closing <> 1 or r.total_lead <> 1 then
        raise exception 'TEST FAILED: non-profit fields corrupted, closing=%, total_lead=%', r.closing, r.total_lead;
      end if;
    end if;
  end loop;
  if not v_found then
    raise exception 'TEST FAILED: cs row not found in get_cs_performance at all';
  end if;
  raise notice 'TEST 5 PASSED: cs sees gross_profit=NULL and gross_booking_value=NULL, other fields intact';
end $$;
